...
public class UserServer {
	...
	void ServerOpen() throws Exception {
		...
		try {
			...
			while (true) {
				...
				System.out.println("읽어들인 값 : " + data.get("id") + data.get("password") + data.get("name") + data.get("company") + data.get("flag"));
				db.connectDB();// db와 연결
	...
	{
		int flag = data.get("flag").getAsInt();
		// 아이디 비밀번호 플래그0 들어오면 로그인 , 아이디  플래그1 로그아웃,  4가지 들어오면+플래그가0이면 회원가입,  아이디 비밀번호 플래그1이면 회원탈퇴
		if (flag==0)
		{
			if(data.get("id")!=null&&data.get("password")!=null&&data.get("name")==null&&data.get("company")==null)
			{
